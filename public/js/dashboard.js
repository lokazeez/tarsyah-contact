...
function fetchChartData() {
  ...
  $.ajax({
    ...
    donutChartRender(data, 6); // PieChartRender(data,6);
  }); // $.ajax({
  //     method: 'GET',
...
function bestCustomers() {
  var table = $('#kt_datatable_customers');
  var locale = table.attr('data-locale') === '' ? "en" : table.attr('data-locale');
  var columnsName = {
    'ar': {
      'customer': 'الزبون',
  ...
  var datatable = $('#kt_datatable_customers').KTDatatable({
    // datasource definition
    data: {
      type: 'remote',
      source: {
