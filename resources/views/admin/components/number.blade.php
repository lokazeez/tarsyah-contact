    ...
    <input @if($required) required @endif id="{{ $name }}"  class="form-control form-control-lg form-control-solid @error($name) is-invalid @enderror {{$percentage ?'percentage':''}}"
           placeholder="{{__('admin.enter')}} {{ isset($item) ? __($item.'.'.$name) : __('admin.'.$name)}}"
           name="{{ $name }}" type="number"
           value="{{ $oldValue ? $oldValue->{$name} : old($name) }}"
           min="{{ $min ?? '' }}" max="{{ $max ?? '' }}"
           {{ $readonly ? 'readonly' : '' }}
           @if($decimal == true) step="any" @endif/>
    </div>
    @error($name)
