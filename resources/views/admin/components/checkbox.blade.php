        ...
        <div class="checkbox-inline">
            @foreach($choices as $choice)
                <label class="checkbox checkbox-square checkbox-light-success">
                    <input id="{{ $name }}" name="{{ $name }}" {{ $oldValue ? ($oldValue->{$name} == 1 ? 'checked' : '') : ($checked ? 'checked':'') }} type="checkbox" value="{{ $choice->value }}" @if($required) required @endif/> {{ __('admin.'.$choice->name) }}
                    <span></span>
                </label>
            @endforeach
            @error($name)
